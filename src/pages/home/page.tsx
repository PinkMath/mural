...
import { getMonthSlug } from '@/mocks/photoWall';
...
const Home = () => {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      if (y < heroH) setHeroParallax(y * 0.28);
    };
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-white/30 text-xs tracking-widest uppercase">Carregando...</p>
        </div>
      </div>
    );
  }
  ...
  const scrollToIndex = () => {
    document.getElementById('meses')?.scrollIntoView({ behavior: 'smooth' });
    setMenuOpen(false);
  };
    ...
    <div className="grain-overlay min-h-screen bg-[#0a0a0a] text-white">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-6">
          ...
          <div className="grid grid-cols-3 gap-2 mb-8">
            {months.map((m) => (
              <Link
                key={m.month}
                to={`/${getMonthSlug(m.month)}`}
                onClick={() => setMenuOpen(false)}
                className="flex flex-col items-center gap-1 py-3 border border-white/8 hover:border-[#C2003E]/40 hover:bg-[#C2003E]/5 transition-all cursor-pointer"
              >
                ...
                {m.photos.length > 0 && (
                  <span className="text-[#C2003E]/50 text-[9px]">{m.photos.length} foto{m.photos.length > 1 ? 's' : ''}</span>
                )}
              </Link>
            ))}
          </div>
          <button
            ...
            className="w-full py-3 bg-[#C2003E] text-white text-xs font-bold uppercase tracking-[0.25em] cursor-pointer whitespace-nowrap"
          >
            Ver todos os meses
          </button>
        </div>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-40 transition-all duration-500 ${
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 md:px-16 h-14 md:h-20">
          <div className="flex items-center gap-2.5">
            ...
            />
            <span className="text-white text-sm font-medium tracking-[0.2em] uppercase bebas">Terceirao</span>
          </div>

          <div className="hidden lg:flex items-center gap-5 xl:gap-6">
            {months.slice(0, 6).map((m) => (
              <Link key={m.month} to={`/${getMonthSlug(m.month)}`}
                className="text-white/30 hover:text-[#C2003E] text-[10px] tracking-[0.25em] uppercase transition-colors duration-300 cursor-pointer whitespace-nowrap nav-link pb-1">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={scrollToIndex}
              className="px-4 py-2 border border-[#C2003E]/30 text-[#C2003E] text-xs uppercase tracking-[0.2em] hover:bg-[#C2003E] hover:text-white transition-all duration-300 cursor-pointer whitespace-nowrap hidden md:block">
              Ver Meses
            </button>
            <button
      ...
      <section className="relative w-full h-screen min-h-[580px] overflow-hidden bg-[#0a0a0a]">
        ...
        <div className="absolute left-6 top-1/2 -translate-y-1/2 z-20 hidden lg:flex flex-col items-center gap-4">
          <div className="w-px h-16 bg-[#C2003E]/40" />
        ...
        <div className="hidden md:flex absolute inset-0 z-20 flex-col justify-end pb-16 lg:pb-20 px-12 lg:px-24 pointer-events-none"
          ...
          <div className={`transition-all duration-1000 ${loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
            ...
            <div className="flex items-center gap-3 mb-3">
              <div className="w-8 h-px bg-[#C2003E]" />
              <span className="text-[#C2003E] text-xs uppercase tracking-[0.3em]">Nosso último ano na escola</span>
            </div>
            ...
            <button onClick={scrollToIndex} className="pointer-events-auto px-8 py-3 bg-[#C2003E] text-white text-xs font-bold uppercase tracking-[0.25em] hover:bg-white transition-colors duration-300 cursor-pointer whitespace-nowrap">
              Ver Meses
            </button>
          </div>
        </div>
        ...
        <div className={`hidden md:flex absolute top-24 right-12 lg:right-16 z-20 flex-col items-end gap-5 transition-all duration-1000 ${loaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}
          style={{ transitionDelay: '0.7s', transform: `translateY(${heroParallax * 0.18}px)`, willChange: 'transform' }}>
        ...
        <div className="hidden md:flex absolute bottom-6 right-12 lg:right-16 z-20 flex-col items-center gap-2 cursor-pointer pointer-events-auto" onClick={scrollToIndex}
          style={{ transform: `translateY(${heroParallax * 0.12}px)`, willChange: 'transform' }}>
          <div className="w-11 h-11 rounded-full border border-[#C2003E]/40 flex items-center justify-center animate-float hover:border-[#C2003E] transition-colors duration-300">
            <i className="ri-arrow-down-line text-[#C2003E] text-lg" />
          </div>
        ...
        <div className={`md:hidden absolute inset-0 z-20 flex flex-col justify-end px-5 pb-12 pointer-events-none transition-all duration-1000 ${loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
          style={{ transitionDelay: '0.3s', transform: `translateY(${heroParallax * 0.22}px)`, willChange: 'transform' }}>
          <div className="flex items-center gap-2 mb-3">
            <div className="w-5 h-px bg-[#C2003E]" />
            <span className="text-[#C2003E] text-[10px] uppercase tracking-[0.3em]">Nosso último ano</span>
          </div>
          ...
          <div className="flex items-center gap-3 pointer-events-auto">
            <button onClick={scrollToIndex}
              className="flex-1 py-3 bg-[#C2003E] text-white text-xs font-bold uppercase tracking-[0.2em] cursor-pointer whitespace-nowrap">
              Ver Meses
            </button>
      ...
      {/* ── ÍNDICE DE MESES ── */}
      <section id="meses" className="px-4 sm:px-6 md:px-16 lg:px-24 py-14 md:py-24">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10 md:mb-16 border-b border-white/5 pb-8 md:pb-12">
          <div>
            <div className="flex items-center gap-3 mb-3">
              <div className="w-6 md:w-8 h-px bg-[#C2003E]" />
              <span className="text-[#C2003E] text-[10px] uppercase tracking-[0.4em]">Arquivo do Terceirao</span>
            </div>
        ...
        {/* Month list — clean, no photos */}
        <div className="flex flex-col divide-y divide-white/5">
          {months.map((m, i) => {
            const slug = getMonthSlug(m.month);
            const hasPhotos = m.photos.length > 0;
            return (
              <Link
                key={m.month}
                to={`/${slug}`}
                className="group flex items-center gap-4 sm:gap-6 py-5 sm:py-6 hover:bg-white/[0.02] transition-colors duration-300 cursor-pointer"
                style={{
                  opacity: 0,
              ...
              >
                {/* Number */}
                <span className="bebas text-white/8 group-hover:text-white/14 text-4xl sm:text-5xl tabular-nums leading-none w-14 sm:w-16 flex-shrink-0 transition-colors duration-300">
                  {String(m.month).padStart(2, '0')}
                </span>

                {/* Divider */}
                <div className="w-px h-10 bg-white/6 flex-shrink-0 group-hover:bg-[#C2003E]/20 transition-colors duration-300" />

                {/* Name */}
                <div className="flex-1 min-w-0">
                  <h3 className="bebas text-white/60 group-hover:text-white text-2xl sm:text-3xl md:text-4xl tracking-[0.05em] leading-none transition-colors duration-300">
                    {m.label}
                  </h3>
                  <p className="text-white/20 text-[10px] tracking-[0.3em] uppercase mt-1">
                    {m.year}
                    {hasPhotos && (
                      <> · <span className="text-[#C2003E]/40">{m.photos.length} {m.photos.length === 1 ? 'foto' : 'fotos'}</span></>
                    )}
                    {!hasPhotos && <> · <span className="text-white/15">sem fotos</span></>}
                  </p>
                </div>

                {/* Arrow */}
                <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center border border-white/8 group-hover:border-[#C2003E]/40 group-hover:text-[#C2003E] text-white/20 transition-all duration-300 group-hover:translate-x-1">
      ...
      {/* Footer */}
      <footer className="border-t border-[#C2003E]/10 py-8 md:py-10 px-4 sm:px-6 md:px-16 bg-[#060606]">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <img
              src="https://storage.readdy-site.link/project_files/7bbe0463-89d4-4e9b-9a6b-a09970c7f4c9/ed7df51e-ad2c-4e05-8a65-2c6744e8b1ef_file.jpg?v=6c13df9251a5acd90fc6079411dcefdf"
              alt="Terceirao"
              className="w-7 h-7 object-contain rounded-sm opacity-50"
            />
          ...
          <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
            className="group text-white/20 hover:text-[#C2003E] text-xs tracking-[0.3em] uppercase transition-colors cursor-pointer flex items-center gap-2 whitespace-nowrap">
