...
interface WorkArchiveGalleryProps {
  workId: number | null;
  onClose: () => void;
}

const WorkArchiveGallery = ({ workId, onClose }: WorkArchiveGalleryProps) => {
  ...
  return (
    ...
    >
      {/* Top bar */}
      <div className="flex items-center justify-between px-6 md:px-12 py-5 border-b border-white/5 flex-shrink-0">
        <div>
          <span className="text-[#00ffcc] text-[10px] uppercase tracking-[0.35em] block mb-0.5">
            {work.category} — {work.year}
          </span>
          <h2 className="bebas text-white text-2xl md:text-4xl leading-none tracking-wide">{work.title}</h2>
        </div>
      ...
      {/* Main image */}
      <div className="flex-1 relative flex items-center justify-center overflow-hidden px-4 py-4">
        <img
          key={activeIndex}
      ...
      {/* Thumbnail strip */}
      <div className="flex-shrink-0 px-6 md:px-12 py-4 border-t border-white/5">
...
export default WorkArchiveGallery;
