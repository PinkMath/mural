...
const PhotoUploader = ({ monthLabel, onPhotosReady }: Props) => {
  ...
  const getCaptionFromDate = async (file: File, fallbackName: string) => {
    ...
    return date ? `Foto do dia ${date}` : fallbackName.replace(/\.[^.]+$/, '');
  };
  ...
  const processFiles = async (files: File[]) => {
    ...
    for (let i = 0; i < imageFiles.length; i++) {
      ...
      if (uploadedKeysRef.current.has(fileKey)) {
        updateProgress(i, 'skipped');
        continue;
      }

      uploadedKeysRef.current.add(fileKey);
      updateProgress(i, 'processing');

      try {
        const caption = await getCaptionFromDate(file, file.name);
        ...
        if (err instanceof Error) {
          setError(`Erro em ${file.name}: ${err.message}`);
        }
      ...
      for (let i = 0; i < imageEntries.length; i++) {
        ...
        try {
          ...
          const imageFile = new File([blob], filename, {
            type: blob.type || (/\.(heic|heif)$/i.test(filename) ? 'image/heic' : 'image/jpeg'),
          });

          const fileKey = `${filename}-${blob.size}`;
          ...
          const caption = await getCaptionFromDate(imageFile, filename);
  ...
  const handleDragOver = (e: DragEvent) => {
    ...
    if (items.length > 0) {
      setPreviewInfo(`${items.length} arquivo(s) detectado(s)`);
    }
  };
      ...
      {!processing && !done && (
        ...
        >
          ...
          <div className="text-center">
            ...
            {previewInfo && (
              <p className="text-[#C2003E]/70 text-xs mt-2">
                {previewInfo}
              </p>
            )}
          </div>
          ...
          {progress.map((item, i) => (
            <div key={`${item.name}-${i}`} className="flex items-center gap-2 px-2 py-1">
              ...
              >
                ...
                {item.status === 'skipped' && <i className="ri-skip-forward-line" />}
              </span>

              <p className="text-white/40 text-xs truncate flex-1">
                {item.name}
                {item.status === 'skipped' && (
                  <span className="text-blue-400/60 ml-2">duplicada ignorada</span>
                )}
              </p>
            </div>
          ))}
      ...
      {processing && progress.length > 0 && (
        <div className="mb-3">
          ...
          <button
            ...
            className="mt-3 border border-red-500/30 text-red-400 hover:bg-red-500/10 text-xs uppercase tracking-[0.2em] px-4 py-2 transition-colors cursor-pointer"
          >
            Cancelar upload
          </button>
        </div>
        ...
        <div className="flex items-center justify-between bg-[#C2003E]/10 border border-[#C2003E]/20 px-3 py-2">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[#C2003E] text-xs">
              ...
              {skippedCount > 0 && (
                <span className="text-blue-400 ml-1">({skippedCount} duplicada{skippedCount !== 1 ? 's' : ''} ignorada{skippedCount !== 1 ? 's' : ''})</span>
              )}
            </p>
          </div>
